...
- **Twelve new UI locales: Arabic, Bengali, Gujarati, Hindi, Indonesian, Marathi, Russian, Tamil, Telugu, Thai, Turkish and Urdu.** The extension's own interface is now available in 22 languages (was 10). Added together with a locale parity gate (`tests/locales.test.js`) and `scripts/locale-status.mjs`, a per-locale report of missing keys and strings still identical to English. The translations are AI-produced and structurally verified; native review is welcome.
- Backfilled the 14 message keys that `de es fr it ja ko pt zh` were each missing, so all 22 locales now define the same 182 keys.
