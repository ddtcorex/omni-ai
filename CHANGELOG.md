...
- **Aligned the three Custom Gateway sentences across all 46 catalogues.** `settings_customGatewayHint`, `settings_customGatewayUrlNote` and `settings_customGatewayKeyNote` were still English in `de es fr it ja ko pt zh` (inherited from the English file when those catalogues were behind) while the other 37 translated them. The i18n carve-out covers brand names, not whole sentences, so the sentences are now translated everywhere with `Custom Gateway`, `OpenRouter`, `LiteLLM` and `Together AI` kept in Latin script. A test asserts it stays that way.
