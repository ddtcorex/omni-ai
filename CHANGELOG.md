...
- **Aligned six Custom Gateway messages across all catalogues.** `settings_customGatewayHint`, `settings_customGatewayUrlNote` and `settings_customGatewayKeyNote` were still English in `de es fr it ja ko pt zh` (inherited from the English file when those catalogues were behind) while the other 37 translated them. The i18n carve-out covers brand names, not whole sentences, so all six messages are now translated everywhere with `Custom Gateway`, `OpenRouter`, `LiteLLM` and `Together AI` kept in Latin script. That covers three sentences and three short strings (`settings_customGatewayUrlPlaceholder`, `settings_customGatewayKeyPlaceholder`, `settings_tooltip_customGateway`) which were the same kind of inherited English. A test asserts it stays that way.
- **The overlay's language chip no longer strands a preposition in front of the language name.** `ui_to` was a bare "To" rendered as `${flag} To ${languageName}`, which postpositional and verb-final languages cannot phrase correctly; the six wave-2 translators each worked around it differently (`→`, `Hedef:`, `లోకి`, `เป็นภาษา`). It is now a `$LANGUAGE$` placeholder message, so each locale orders the name and any marker around it. Both i18n wrappers (`lib/i18n.js` and the content script's own) now apply a message's `placeholders` map themselves, because they read `_locales/*/messages.json` directly and would otherwise render a literal `$LANGUAGE$`.
- **Two more UI strings no longer glue a value onto a translated fragment.** `error_apiKeyNotConfiguredFor` is now a `$MODEL$` placeholder message and the model id is passed as an i18n substitution, and the provider tooltip's first step is one message inside the link (`Go to Google AI Studio`) instead of a bare "Go to" followed by the anchor. Concatenating a translated fragment with a value hard-codes English word order, which reads wrong in verb-final languages (Hindi, Urdu, Bengali, and the Indic languages still to come). The four now-unused standalone brand keys were removed, which is why the catalogs carry 182 keys instead of 186.
...
- Removed every em dash, en dash and punctuation spaced hyphen from documentation, code comments and review templates across the repository.
...
- The result card's "Back" button now returns to the action menu after a keyboard-shortcut-triggered result (Alt+R/T/F), not just after a click-triggered one.
- Settings: the Validate Configuration status card no longer renders as a persistently-visible empty box, and the Save Settings button no longer loses its color or full-width styling.
- The Quick Ask overlay's quoted-context card no longer sits flush against the reply text or the input field below it.
