...
/**
 * Models retired from the curated catalog but still stored as `apiModel` by
 * existing users. Each entry carries both the provider that owns it and its
 * real (pre-retirement) API model name, so routing, key lookup and request
 * building all keep working — not just the Settings dropdown's display.
 * Future catalog refreshes append here rather than replacing entries.
 * @type {Record<string, { provider: string, apiModel: string }>}
 */
export const LEGACY_MODELS = {
  "gemini-2.0-flash": { provider: "google", apiModel: "gemini-2.0-flash" },
  "gemini-2.5-flash-lite": { provider: "google", apiModel: "gemini-2.5-flash-lite" },
  ...
  "openai-gpt-4-turbo": { provider: "openai", apiModel: "gpt-4-turbo" },
...
export function getProviderByModel(modelId) {
  ...
  // Fallback: a model retired from the curated catalog but still stored by an
  // existing user. Without this, routing would silently fall back to Gemini.
  const legacy = LEGACY_MODELS[modelId];
  return legacy ? AI_PROVIDERS[legacy.provider] || null : null;
}
...
export function getApiModelName(modelId) {
  ...
  // 3. Legacy model: retired from the curated catalog but still functional —
  //    resolve it to its real API name rather than its retired display id.
  const legacy = LEGACY_MODELS[modelId];
  if (legacy) {
    return legacy.apiModel;
  }
