...
export async function generateContent(prompt, config) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let errorMessage = `Ollama API Error (${response.status}): ${errorText || response.statusText}`;

      if (response.status === 403) {
        errorMessage +=
          '\n\nHint: This is likely a CORS issue. Please set OLLAMA_ORIGINS="chrome-extension://*" on your Ollama server.';
      }
