...
/**
 * A single usage-history entry as persisted under `usageHistory` in
 * chrome.storage.local. Shape derives from createHistoryEntry (producer)
 * plus every field read by consumers (getHistoryByAction -> action,
 * searchHistory -> inputText/outputText, deleteOldEntries -> timestamp,
 * updateStats -> action/wordCount/outputWordCount).
 *
 * @typedef {Object} HistoryEntry
 * @property {string} id - Generated id ("hist_<base36 ts>_<rand>").
 * @property {string} action - Action identifier (e.g. "improve", "translate").
 * @property {string} inputText - Input text truncated to 200 chars.
 * @property {string} outputText - Output text truncated to 500 chars.
 * @property {string} preset - Preset name; defaults to "email".
 * @property {string} site - Site origin; "" when unknown.
 * @property {number} timestamp - Creation time, epoch milliseconds.
 * @property {number} wordCount - Word count of inputText.
 * @property {number} outputWordCount - Word count of outputText.
 */
...
export async function getHistory() {
  ...
  return /** @type {HistoryEntry[]} */ (result[HISTORY_KEY]) || [];
}

/**
 * Get history filtered by action type
 * @param {string} action - Action type
 * @returns {Promise<HistoryEntry[]>} Filtered entries
 */
 ...
 * @returns {Promise<HistoryEntry[]>} Matching entries
 */
...
function generateId() {
  return "hist_" + Date.now().toString(36) + "_" + Math.random().toString(36).substr(2, 9);
}
