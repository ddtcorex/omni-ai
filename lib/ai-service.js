...
async function initSettings() {
  if (cachedSettings.initialized) return;

  if (!initPromise) {
    initPromise = (async () => {
      const keys = ['apiKey', 'groqApiKey', 'apiModel'];
      ...
      // Store any other keys we might have missed if we expand this later
      // For now, the explicit keys are enough.
        ...
        chrome.storage.onChanged.addListener((changes, area) => {
          if (area === "local") {
            for (const [key, val] of Object.entries(changes)) {
               cachedSettings[key] = val.newValue || "";
            }
          }
        });
      }
...
async function getProviderKey(keyName) {
  if (!cachedSettings.initialized) await initSettings();

  if (cachedSettings[keyName] !== undefined) {
    return cachedSettings[keyName];
  }

  // Fallback for unknown keys
  const result = await chrome.storage.local.get(keyName);
  const val = result[keyName] || "";
  cachedSettings[keyName] = val;
  return val;
}

async function getModel() {
  if (!cachedSettings.initialized) await initSettings();
  return cachedSettings.apiModel;
}
