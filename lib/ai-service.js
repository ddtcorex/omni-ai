...
export async function generateContent(prompt, options = {}) {
  ...
  if (!apiKey) {
    ...
    if (!isCustomGatewayModel) {
      // The model id travels as a substitution so each locale can place it
      // wherever its own word order needs it ($MODEL$ in the message).
      throw new Error(chrome.i18n.getMessage("error_apiKeyNotConfiguredFor", [activeModel]));
    }
  }
  ...
  if (isCG) {
    ...
    if (options.baseUrl) {
      ...
      const gwConfig = await getCustomGatewayConfig();
