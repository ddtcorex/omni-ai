...
export async function generateContent(prompt, options = {}) {
  ...
  if (!apiKey) {
    if (activeModel.startsWith("groq-")) {
      // Actually this block is now redundant if we trust providerSettings, but let's stick to the Refactor plan
      // The previous refactor step introduced getProviderKey call.
      // We should use that logic but allow override.
      
      const providerSettings = getProviderByModel(activeModel);
      if (!providerSettings) {
        throw new Error(`Unknown model provider: ${activeModel}`);
      }
      apiKey = await getProviderKey(providerSettings.keySetting);
    } else {
        // Fallback or specific logic if needed, but getProviderByModel covers it
        const providerSettings = getProviderByModel(activeModel);
        if (providerSettings) {
            apiKey = await getProviderKey(providerSettings.keySetting);
        } else {
             // Fallback legacy (shouldn't happen with new system)
