    ...
    initPromise = (async () => {
      ...
      if (chrome.storage.onChanged) {
        chrome.storage.onChanged.addListener((changes, area) => {
          if (area === "local") {
            if (changes.apiKey)
              cachedSettings.apiKey = changes.apiKey.newValue || "";
            if (changes.groqApiKey)
              cachedSettings.groqApiKey = changes.groqApiKey.newValue || "";
            if (changes.apiModel)
              cachedSettings.apiModel = changes.apiModel.newValue || "";
          }
        });
      }
      initPromise = null;
...
async function getGroqApiKey() {
  if (!cachedSettings.initialized) await initSettings();
  return cachedSettings.groqApiKey;
}
