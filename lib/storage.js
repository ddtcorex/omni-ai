...
/**
 * Notify a callback whenever primaryLanguage changes in chrome.storage.sync
 * (e.g. Settings' Save button), so a caller with its own primaryLanguage-
 * derived state (content.js's own overlay locale) can refresh it live instead
 * of only picking up the new value on next page load.
 * @param {() => void} callback
 */
export function onPrimaryLanguageChanged(callback) {
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === "sync" && changes.primaryLanguage) callback();
  });
}
